...
// 메서드
// 1. test()
// 문자열이 정규표현식과 일치하는지 검사하고 결과를 true 또는 false로 반환
let pattern1 = /abc/g;
console.log(pattern1.test("ABC"));
console.log(pattern1.test("abc"));

// 2.exec() : 매칭 결과 상세 정보.
// 문자열에서 정규표현식과 일치하는 문자열을 검색하고, 일치하는 문자열을 배열로 반환
pattern1 = /hello/g;
console.log(pattern1.exec("hello world")); // index: 0
console.log(pattern1.exec("hello world hello weniv")); // index: 12. 마지막인덱스를 기억했다가 다시 이어서 검색 매칭을 계속 이어가다 마지막에 없으면 null이 되고 최종값 출력
