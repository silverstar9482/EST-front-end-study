...
const friend3 = new Friends("빙키");
console.log(friend3.name);

// 8. 이벤트 핸들러
// Element를 가져왔다고 명시할때 $를 붙여줌(명령어는 아님)
const $button = document.querySelector("#button");

// 8-1. 일반 함수
// this: 누가 이벤트를 받았는지, 실제 클릭된 DOM 요소
$button.addEventListener("click", function () {
    console.log("이벤트 핸들러(일반)", this); //<button></button>
    console.log("클릭된 요소", this.tagName);
});

// 8-2. 화살표 함수
$button.addEventListener("click", () => {
    console.log("이벤트 핸들러(화살표)", this); //window <- 상위스코프를 가져오기 때문에 현재 상위스코프는 window이기 때문에 window를 가져옴
    console.log("클릭된 요소", this.tagName);
});
