...
// => 사용자가 직접 지우지 않는 한, 데이터는 브라우저를 꼇다 켜도 그대로 남아있음.
...
// 값 지우기(데이터 삭제)
localStorage.removeItem("user"); // 지정된 값만 삭제
...
// 값 지우기
sessionStorage.removeItem("user"); // 지정된 값만 삭제
...
- 문자로만 저장, 4KB 제한
- 자동 전송 -> 인증 정보에 자주 쓰임.
- HttpOnlyt 속성이 설정된 쿠키는 자바스크립트로 읽을 수 없다 => 보안을 위한 장치
*/
